...
export default function PicksPage() {
  const { upcomingPredictions, isLoading, fetchPredictions, fetchStats, fetchAgentStatus, stats } = useStore();
  ...
  const smartPicks = upcomingPredictions
    .filter(p => (!p.result || p.result.status === 'pending') && p.prediction.hasValueBet)
        ...
        >
          <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
            <div>
              ...
              <p className="text-muted-foreground mt-1">Matches where our AI found the market underpricing a team</p>
            </div>
          </div>
    ...
    <div className="container mx-auto px-4 py-8 md:py-12 min-h-screen">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
           ...
           {smartPicks.length > 0 && (
              <div className="px-4 py-1.5 rounded-full bg-success/10 text-success border border-success/20 text-sm font-bold flex items-center gap-2">
                 <Zap size={14} className="fill-success" />
                 {smartPicks.length} picks today
              </div>
           )}
        </div>
        ...
        {isLoading ? (
           ...
           <div className="py-24 text-center glass-card rounded-2xl border-white/10 flex flex-col items-center justify-center relative overflow-hidden">
             ...
             <h3 className="text-2xl font-bold text-foreground mb-3 text-balance">
                No Smart Picks today — our AI analyzed {Math.max(145, stats.totalPredictions * 4)} matches and found the market has them all fairly priced.
             </h3>
