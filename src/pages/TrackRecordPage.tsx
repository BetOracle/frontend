...
import { Target, TrendingUp, Cpu, Activity, Trophy, BarChart3, Database } from 'lucide-react';
import {
...
type Tab = 'Overview' | 'By League' | 'Recent Record';
...
export default function TrackRecordPage() {
  ...
  const recentRecordTable = [...resolvedPredictions].reverse().slice(0, 50);
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <div className="mb-8 text-center md:text-left">
           <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-2 flex items-center justify-center md:justify-start gap-3">
             <BarChart3 size={32} className="text-primary" /> System Performance
           </h1>
           <p className="text-muted-foreground">Comprehensive track record and analytics of the BetOracle Agent model.</p>
        </div>

        {/* Custom Tabs */}
        <div className="flex bg-white/5 border border-white/10 rounded-xl p-1 w-full md:w-max mx-auto md:mx-0 mb-8 overflow-x-auto glass-card">
          {(['Overview', 'By League', 'Recent Record'] as Tab[]).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={cn(
                "px-5 mx-0.5 py-2 text-sm font-medium rounded-lg transition-all whitespace-nowrap outline-none",
                activeTab === tab 
                  ? "bg-primary/20 text-primary border border-primary/30 shadow-[0_0_15px_rgba(53,208,127,0.15)] glow" 
        ...
        {isLoading ? (
          ...
          <AnimatePresence mode="wait">
            ...
            >
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Card className="glass-card p-6 col-span-1 md:col-span-2 border-white/5">
                       <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground gap-1 flex items-center mb-6"><TrendingUp size={14}/> Edge Growth trajectory</h3>
                           ...
                           <tr key={data.league} className="hover:bg-white/5 transition-colors">
                             ...
                             <td className="p-4 text-center">
                               <div className="flex items-center justify-center gap-2 font-mono text-sm font-semibold">
                                 <span className="text-success">{data.wins}</span><span className="text-muted-foreground font-normal opacity-50">-</span><span className="text-destructive">{data.losses}</span>
                               </div>
                             </td>
              ...
              {activeTab === 'Recent Record' && (
                <Card className="glass-card border-white/5 overflow-hidden p-0 max-h-[600px] overflow-y-auto no-scrollbar">
                   <div className="overflow-x-auto">
                     <table className="w-full text-left border-collapse text-sm">
                       <thead>
                         <tr className="bg-black/40 text-[10px] uppercase text-muted-foreground tracking-widest font-bold border-b border-white/5 sticky top-0 z-10">
                       ...
                       <tbody className="divide-y divide-white/5 bg-background/20">
                         {recentRecordTable.map((pred) => (
                           <tr key={pred.id} className="hover:bg-white/5 transition-colors cursor-pointer" onClick={() => setSelectedPred(pred)}>
                             <td className="p-4 text-muted-foreground whitespace-nowrap">{format(new Date(pred.recordedAt || ''), 'MMM dd')}</td>
                             <td className="p-4">
                               <div className="font-semibold">{pred.match.homeTeam} vs {pred.match.awayTeam}</div>
                               <div className="text-[10px] text-muted-foreground uppercase">{pred.match.league}</div>
                             </td>
                             <td className="p-4 text-center">
                               {pred.prediction.outcome === 'home_win' ? 'HOME' : pred.prediction.outcome === 'away_win' ? 'AWAY' : 'DRAW'}
                             </td>
                             <td className="p-4 text-center font-mono text-primary">
                               +{pred.prediction.edge.toFixed(1)}%
                             </td>
                             <td className="p-4 text-right">
                               <div className={cn("inline-flex items-center justify-center font-bold font-mono text-xs w-8 h-8 rounded-md ring-1", 
                                 pred.result?.status === 'win' ? 'bg-success/20 ring-success/50 text-success glow border border-success/30' : 'bg-destructive/10 ring-destructive/30 text-destructive border border-destructive/20'
                               )}>
                                 {pred.result?.status === 'win' ? 'W' : 'L'}
                               </div>
                             </td>
                           </tr>
                         ))}
